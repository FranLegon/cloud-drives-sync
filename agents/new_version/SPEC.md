...
| 25 | Cross-provider duplicate nested folders with file conflicts | "manually" create folder `test-case-id-25-outer` in `cloud-drives-sync-root` using Google Drive main account, then create folder `test-case-id-25-inner` inside it. Create file `test-case-id-25-outer.txt` inside `test-case-id-25-outer` with content `Uploaded to provider: Google Drive\nOuter`, and file `test-case-id-25-inner.txt` inside `test-case-id-25-inner` with content `Uploaded to provider: Google Drive\nInner`. Then, using a Microsoft OneDrive backup account, create folder `test-case-id-25-outer` in `cloud-drives-sync-root` and folder `test-case-id-25-inner` inside it. Create file `test-case-id-25-outer.txt` inside `test-case-id-25-outer` with content `Uploaded to provider: Microsoft OneDrive\nOuter`, and file `test-case-id-25-inner.txt` inside `test-case-id-25-inner` with content `Uploaded to provider: Microsoft OneDrive\nInner`. Then run sync. | On every provider and account, there is exactly one folder at path `cloud-drives-sync-root/test-case-id-25-outer` and exactly one folder at path `cloud-drives-sync-root/test-case-id-25-outer/test-case-id-25-inner`. Across those merged folders, all four files exist after sync: both versions of `test-case-id-25-outer.txt` are present in `test-case-id-25-outer`, and both versions of `test-case-id-25-inner.txt` are present in `test-case-id-25-outer/test-case-id-25-inner`, with one version of each conflicting filename renamed using the conflict format. File contents remain provider-specific and are not overwritten. In `cloud-drives-sync-metadata.db`, the logical_folder and folder_replica rows represent the merged folder paths, and there are distinct logical_file rows for each conflicting content variant with correct provider fingerprints. |
| 26 | Multiple same-path conflicts collapse into timestamped variants | "manually" create folder `test-case-id-26` in `cloud-drives-sync-root` using Google Drive main account. Inside it, create four files with the same name `test-case-id-26.txt` but different contents: version A, version B, version C, and version D. Then, using a Microsoft OneDrive backup account, create folder `test-case-id-26` in `cloud-drives-sync-root` and create `test-case-id-26.txt` there only once with the content of version C. Then run sync. | On every provider and account, there is exactly one folder at path `cloud-drives-sync-root/test-case-id-26`. After sync, all four content variants exist in that folder: exactly one file keeps the base name `test-case-id-26.txt`, and the other three are renamed using the conflict format `_conflict_YYYY-MM-DD_hh-mm-ss` inserted before the file extension, with three distinct timestamps. The Microsoft-created version C is matched as one of the four variants rather than duplicated into a fifth copy. In `cloud-drives-sync-metadata.db`, there are four distinct logical_file rows with different Google Drive MD5s and replica rows with the correct provider fingerprints and final paths. |
| 27 | Google Drive same-content duplicates collapse to earliest main copy | "manually" create folder `test-case-id-27` in `cloud-drives-sync-root` using Google Drive main account. Inside it, create subfolders `test-case-id-27-A`, `test-case-id-27-B`, and `test-case-id-27-C`. In `test-case-id-27-A`, create `test-case-id-27-a1.txt` and `test-case-id-27-a2.txt`. In `test-case-id-27-B`, create `test-case-id-27-b.txt`. In `test-case-id-27-C`, create `test-case-id-27-c.txt`. All three files must have identical content bytes. Then run sync. | On every provider and account, only one logical file remains for that identical-content set. The surviving canonical file is the copy from Google Drive main account with the earliest `modifiedOn` timestamp, and its filename and logical path are propagated to all replicas. The other same-content variants are removed rather than preserved as conflicts because their content hashes match, their empty folders remain. In `cloud-drives-sync-metadata.db`, there is exactly one logical_file row for that content hash and replica rows only for the surviving canonical path; no extra logical_file rows remain for the duplicate same-hash variants. |
| 28 | Cross-provider same-content duplicates collapse to earliest main copy | "manually" create folder `test-case-id-28` in `cloud-drives-sync-root`. Using Google Drive main account, create folder `test-case-id-28-main` and file `test-case-id-28-main.txt` inside it. Using one Google Drive backup account, create folder `test-case-id-28-gbackup` and file `test-case-id-28-gbackup.txt` inside it. Using one Microsoft OneDrive backup account, create folder `test-case-id-28-msbackup` and file `test-case-id-28-msbackup.txt` inside it. All three files must have identical content bytes. Then run sync. | On every provider and account, only one logical file remains for that identical-content set. The surviving canonical copy is the one originating from Google Drive main account with the earliest `modifiedOn` timestamp among the Google Drive main-account duplicates. Same-content copies from Google Drive backup accounts and Microsoft OneDrive are treated as duplicates of that canonical file and are removed rather than renamed as conflicts, their empty folders remain. In `cloud-drives-sync-metadata.db`, there is exactly one logical_file row for the shared content hash, replica rows point to the final canonical path, and no duplicate logical_file rows remain for same-content variants created across providers. |
